...
public class RSFont extends DrawingArea {
    ...
    public RSFont(boolean TypeFont, String s, Archive archive) {
        ...
        for (int l = 0; l < 256; l++) {
            ...
            int charWidth = characterWidths[l] = stream_1.readUnsignedShort();
            int charHeight = characterHeights[l] = stream_1.readUnsignedShort();
            int k1 = stream_1.readUnsignedByte();
            int l1 = charWidth * charHeight;
            ...
            if (k1 == 0) {
                ...
                for (int j2 = 0; j2 < charWidth; j2++) {
                    for (int l2 = 0; l2 < charHeight; l2++) {
                        fontPixels[l][j2 + l2 * charWidth] = stream.readSignedByte();
                    }
                }
            }
            if (charHeight > baseCharacterHeight && l < 128) {
                baseCharacterHeight = charHeight;
            }
            characterDrawXOffsets[l] = 1;
            characterScreenWidths[l] = charWidth + 2;
            int k2 = 0;
            for (int i3 = charHeight / 7; i3 < charHeight; i3++) {
                k2 += fontPixels[l][i3 * charWidth];
            }
            ...
            for (int j3 = charHeight / 7; j3 < charHeight; j3++) {
                k2 += fontPixels[l][(charWidth - 1) + j3 * charWidth];
            }
            ...
            if(characterDrawYOffsets[l] < var10 && charHeight != 0) {
                var10 = characterDrawYOffsets[l];
             }
             if(characterDrawYOffsets[l] + charHeight > var9) {
                var9 = characterDrawYOffsets[l] + charHeight;
             }
        }
        if (TypeFont) {
        ...
        anInt2656 = baseCharacterHeight - var10;
        anInt2641 = var9 - baseCharacterHeight;
    }
        ...
        for (int currentCharacter = 0; currentCharacter < string.length(); currentCharacter++) {
            ...
            if (character == 60) {
                ...
                if (character == 62 && startIndex != -1) {
                    ...
                    startIndex = -1;
                    if(effectString.equals("lt")) {
                        character = 60;
                     } else {
                    	 if(!effectString.equals("gt")) {
            ...
            if (character == 60) {
                ...
                if(character == 62 && startIndex != -1) {
                    ...
                    if(effectString.equals("lt")) {
                       ...
                       if(!effectString.equals("gt")) {
                          if(effectString.startsWith("img=")) {
                             try {
                                 int iconId = Integer.valueOf(effectString.substring(4));
    ...
    public void drawStringRight(String string, int drawX, int drawY, int color,
            int shadow) {
        if (string != null) {
            setColorAndShadow(color, shadow);
            drawBasicString(string, drawX - getTextWidth(string), drawY);
    ...
    public void drawWrappedText(String message, int x, int y, int width, int height, int color, int shadow, int textAlignmentH, int textAlignmentV, int lineSpacing) {
    	if(message == null) {
            return;
         } 
    	setColorAndShadow(color, shadow);
        if(lineSpacing == 0) {
           lineSpacing = baseCharacterHeight;
        }
        int[] var12 = new int[]{width};
        if(height < anInt2656 + anInt2641 + lineSpacing && height < lineSpacing + lineSpacing) {
           var12 = null;
        }
        int lineCount = splitIntoLines(message, var12, splitTextStrings);
        if(textAlignmentV == 3 && lineCount == 1) {
           textAlignmentV = 1;
        }
        int lineOffset;
        int drawYOffset;
        if(textAlignmentV == 0) { // vertical top
           drawYOffset = y + anInt2656;
        } else if(textAlignmentV == 1) { // vertical center
           drawYOffset = y + anInt2656 + (height - anInt2656 - anInt2641 - (lineCount - 1) * lineSpacing) / 2;
        } else if(textAlignmentV == 2) { // vertical bottom?
           drawYOffset = y + height - anInt2641 - (lineCount - 1) * lineSpacing;
        } else {
           lineOffset = (height - anInt2656 - anInt2641 - (lineCount - 1) * lineSpacing) / (lineCount + 1);
           if(lineOffset < 0) {
              lineOffset = 0;
           }
           drawYOffset = y + anInt2656 + lineOffset;
           lineSpacing += lineOffset;
        }
        for(lineOffset = 0; lineOffset < lineCount; ++lineOffset) {
            if(textAlignmentH == 0) { // horizontal left
               drawBasicString(splitTextStrings[lineOffset], x, drawYOffset);
            } else if(textAlignmentH == 1) { // horizontal center
               drawBasicString(splitTextStrings[lineOffset], x + (width - getTextWidth(splitTextStrings[lineOffset])) / 2, drawYOffset);
            } else if(textAlignmentH == 2) { // horizontal right
               drawBasicString(splitTextStrings[lineOffset], x + width - getTextWidth(splitTextStrings[lineOffset]), drawYOffset);
            } else if(lineOffset == lineCount - 1) {
               drawBasicString(splitTextStrings[lineOffset], x, drawYOffset);
            } else {
            //   this.method3116(splitTextStrings[var13], width); no idea right now
               drawBasicString(splitTextStrings[lineOffset], x, drawYOffset);
               anInt4178 = 0;
            }
            drawYOffset += lineSpacing;
         }
    }
    
    public int splitIntoLines(String message, int[] var2, String[] stringArray) {
    	if(message == null) {
    		return 0;
    	}
    	int xOffset = 0;
    	...
    	for (int currentCharacter = 0; currentCharacter < message.length(); currentCharacter++) {
    		char character = message.charAt(currentCharacter);
            if (character > 255) {
                character = 32;
            }
            if (character == 60) {
                ...
                if (character == 62 && startIndex != -1) {
                    String effectString = message.substring(startIndex + 1, currentCharacter);
                    startIndex = -1;
                    string.append('<');
                    string.append(effectString);
                    string.append('>');
                    if(effectString.equals("br")) {
                    	stringArray[lineOffset] = string.substring(subStartIndex, string.length());
                        lineOffset++;
                        subStartIndex = string.length();
                        xOffset = 0;
                        lastStringLength = -1;
                    } else if(effectString.equals("lt")) {
                    	xOffset += getCharacterWidth(60);
                    	character = 60;
                    } else if(effectString.equals("gt")) {
                    	xOffset += getCharacterWidth(62);
                    	character = 62;
                    } else if(effectString.startsWith(startImage)) {
                        try {
                            int iconId = Integer.valueOf(effectString.substring(4));
                            xOffset += chatImages[iconId].maxWidth;
                        } catch (Exception exception) { }
                    }
                    character = 0;
                }
                if(startIndex == -1) {
                	...
                	if(character == 32) {
                		lastStringLength = string.length();
                        lastXoffset = xOffset;
                        subShift = 1;
                    }
                    if(var2 != null && xOffset > var2[lineOffset < var2.length ? lineOffset : var2.length - 1] && lastStringLength >= 0) {
                    	stringArray[lineOffset] = string.substring(subStartIndex, lastStringLength - subShift);
                        lineOffset++;
                        subStartIndex = lastStringLength;
                        lastStringLength = -1;
                        xOffset -= lastXoffset;
                    }
                    if(character == 45) {
        ...
        if(finalString.length() > subStartIndex) {
        	stringArray[lineOffset++] = finalString.substring(subStartIndex, finalString.length());
        }
    	return lineOffset;
     }
