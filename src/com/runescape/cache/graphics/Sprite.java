...
public final class Sprite extends DrawingArea implements Cloneable {
	...
	public void cleanMinimapDrawingArea() {
	...
	public void trim() {
		...
		for (int y = 0; y < myHeight; y++) {
			for (int x = 0; x < myWidth; x++) {
				newPixels[(y + drawOffsetY) * maxWidth + (x + drawOffsetX)] = myPixels[y * myWidth + x];
			}
		}
		myPixels = newPixels;
		myWidth = maxWidth;
	...
	public void shapeImageToPixels(int i, int j, int ai[], int k, int ai1[], int i1, int j1, int k1, int l1, int i2) {
		try {
			int j2 = -l1 / 2;
			int k2 = -i / 2;
	...
	public void method354(Background background, int i, int j) {
		...
		if (!(j1 <= 0 || i1 <= 0)) {
			method355(myPixels, j1, background.myPixels, i1, DrawingArea.pixels, 0, k1, k, l1, l);
		}
	}
	...
	@Override
	public Sprite clone() {
		Sprite cloneSprite = new Sprite(this.myWidth, this.myHeight);
		cloneSprite.myPixels = this.myPixels;
		cloneSprite.myWidth = this.myWidth;
		cloneSprite.myHeight = this.myHeight;
		cloneSprite.drawOffsetX = this.drawOffsetX;
		cloneSprite.drawOffsetY = this.drawOffsetY;
		cloneSprite.maxWidth = this.maxWidth;
		cloneSprite.maxHeight = this.maxHeight;
		return cloneSprite;
	}
