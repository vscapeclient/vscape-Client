...
public final class DefaultFrame extends GameFrame {
    
    private final static String archive_name = "frame_default";
    ...
    public void init() {
        ...
        //top right
        Sprite redStone_2 = redStone_0.clone();
        redStone_2.flipHorizontal();
        //bottom left
        Sprite redStone_3 = redStone_0.clone();
        redStone_3.flipVertical();
        //bottom right
        Sprite redStone_4 = redStone_3.clone();
        redStone_4.flipHorizontal();
        redstones = new Sprite[]{redStone_0, redStone_1, redStone_2, redStone_3, redStone_4};
    ...
    public void drawRedStones(Client cl) {
        if(Client.showTabComponents && Client.tabID >= 0 && Client.tabID < 14) {
            ...
            if(Client.frameMode == Client.ScreenMode.FIXED) {
                ...
                final boolean largeTab = Client.frameWidth >= Client.smallTabThreshold;
                Sprite redStone = redstones[1];
                if(redStone != null) {
                    int xOffset = largeTab ? Client.frameWidth - 461 : Client.frameWidth - 231;
                    int yOffset = largeTab ? Client.frameHeight - 36 : Client.frameHeight - 73;
                    if (!largeTab && Client.tabID > 6) {
                        yOffset += 36;
                    }
                    xOffset += 33 * (!largeTab ? (Client.tabID % 7) : Client.tabID);
                    redStone.drawSprite(xOffset, yOffset);
                }
            }
        }
    }
    ...
    public void drawTabArea(Client cl) {
        final Rectangle invBounds = getInvBounds();
        final boolean largeTab = Client.frameWidth >= Client.smallTabThreshold;
        if (Client.frameMode == Client.ScreenMode.FIXED) {
            SpriteLoader.getSprite(archive_name, "tabs", 0).drawSprite(0, 0);
            cl.mascotInv.draw24BitSprite(30, 37);
        } else {
            final int xOffset = largeTab ? Client.frameWidth - 461 : Client.frameWidth - 231;
            final int yOffset = largeTab ? Client.frameHeight - 36 : Client.frameHeight - 73;
            SpriteLoader.getSprite(archive_name, "tabs", largeTab ? 3 : 2).drawSprite(xOffset, yOffset);
            if (Client.showTabComponents) {
                final int xOffSp = Client.frameWidth - ((int)invBounds.getWidth() + (largeTab ? 0 : 14));
                final int yOffSp = (int)invBounds.getHeight();
                final int xOffBg = xOffSp + 7;
                final int yOffBg = yOffset - (yOffSp-7);
                DrawingArea.method335(0x50463C, yOffset - yOffSp, invBounds.width-1, invBounds.height-1, 200, xOffSp);
                cl.mascotInv.draw24BitSprite(xOffBg, yOffBg);
                SpriteLoader.getSprite(archive_name, "tabs", 1).drawSprite(xOffSp, yOffset - yOffSp);
            }
        }
    ...
    public void processTabClick(Client cl) {
        ...
        if(Client.frameMode == Client.ScreenMode.FIXED) {
            ...
            boolean largeTab = Client.frameWidth >= Client.smallTabThreshold;
            final int baseX = largeTab ? Client.frameWidth - 461 : Client.frameWidth - 231;
            final int baseY = largeTab ? Client.frameHeight - 36 : Client.frameHeight - 73;
    ...
    public void drawChatArea(Client cl) {
        final int yOffset = getChatOffsetY(false);
        if (Client.showChatComponents) {
            if (Client.resizableChatArea) {
                DrawingArea.method335(0xccbb9a, yOffset, 520, 143, 200, 0);
                SpriteLoader.getSprite(archive_name, "chat", 1).drawSprite(0, yOffset);
        ...
        if (Client.resizableChatArea) {
            SpriteLoader.getSprite(archive_name, "chat", 2).drawSprite(0, Client.frameHeight - 23);
        }
    ...
    public void drawChannelButtons(Client cl) {
        ...
        if(cl.chatTypeIndex >= 0 & cl.chatTypeIndex <= 6) {
            SpriteLoader.getSprite(archive_name, "chat", 4).drawSprite(cbX[cl.chatTypeIndex], 142 + yOffset);
        }
        if(cl.cButtonHPos >= 0 & cl.cButtonHPos <= 6) {
            SpriteLoader.getSprite(archive_name, "chat", cl.cButtonHPos == cl.chatTypeIndex ? 5 : 3).drawSprite(cbX[cl.cButtonHPos], 142 + yOffset);
        }
    ...
    private final static int[] stoneType = {
            0, 1, 1, 1, 1, 1, 2, 3, 1, 1, 1, 1, 1, 4
    };
    private final static int[] stoneX = {
            6, 44, 77, 110, 143, 176, 209
    };
    private final static int[] stoneY = {
            0, 0, 0, 0, 0, 0, 0, 298, 298, 298, 298, 298, 298, 298
    };
    //Sideicons
    private final static int[] siX = {
            11, 5, 6, 4, 4, 4, 5, 8, 5, 5, 6, 5, 8, 9
    };
    private final static int[] siY = {
            9, 7, 7, 5, 2, 3, 7, 5, 8, 8, 4, 6, 4, 5
    };
    private final static int[] siRX = {
            7, 5, 6, 3, 4, 4, 3, 6, 5, 5, 6, 5, 8, 7
    };
    private final static int[] siRY = {
            7, 6, 6, 4, 1, 2, 5, 5, 6, 6, 2, 5, 4, 4
    };
    //Chat buttons
    ...
    //Chat bounds
    private final static Rectangle chatBounds = new Rectangle(10,8,497,112);
    //Inventory Bounds
